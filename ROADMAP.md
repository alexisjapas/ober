...
- [x] **Explorateur de fichiers intégré** (egui) : navigation dossiers/fichiers audio, chargement direct → deck A/B, touche `B`, ouvert par LOAD/`F`/`L`/bouton MIDI Load ; le dialogue système `rfd` (§6.3, xdg-portal sans GTK) reste accessible depuis sa barre d'outils
