...
/// `sample_rate` : cible de décodage (fréquence du stream ouvert).
pub fn open(deck: Deck, tx: Sender<WorkerMsg>, sample_rate: u32) {
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            if let Some(path) = file {
                spawn_load_worker(path, deck, tx, sample_rate);
            }
        })
        .ok();
}
